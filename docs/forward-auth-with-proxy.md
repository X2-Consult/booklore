...
Trove supports **Forward Auth**, allowing you to specify when a user is logged in using a reverse proxy and existing SSO provider.

## ⚠️ Security

** Important**: Enabling forward auth means Trove will **fully trust headers sent by the reverse proxy**. Never expose Trove directly to the internet when using forward auth - always route through your authenticated proxy, otherwise outsiders can attempt to impersonate any username they know about.

## Configuration

Provide Trove with the following environment variables:
...
1. **Access Admin Settings**: Log in to Trove as an admin user
